...
import { ROLE_LABELS, can, currentUser, requests as allRequests, sortedByLastName } from '../lib/derive'
            ...
            <dd>
              {staffCanWork ? (
                <select className="inline-select" value={r.assigneeId ?? ''} onChange={e => {
                  ...
                  {sortedByLastName(state.users.filter(u => u.orgId === state.currentOrgId && u.status !== 'revoked')).map(u => <option key={u.id} value={u.id}>{u.name} · {ROLE_LABELS[u.role]}</option>)}
                </select>
